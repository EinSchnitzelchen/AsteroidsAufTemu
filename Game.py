...
class Asteroid(pygame.sprite.Sprite):
    def __init__(self, screen, level, game, size=None, spawned_by_other=False, x=0, y=0):
        super().__init__()
        self.size = size if size else r.randint(10, 50)
        self.screen = screen
        self.game = game  
    ...
    def hit(self):
        if self.size > 30:
            ...
            self.game.new_asteroid(self.size, self.posx, self.posy)  # Use game instance
            self.set_random_direction()
...
class Game:
    def __init__(self):
        ...
        self.asteroids = [Asteroid(screen, level, self) for _ in range(5)]
    ...
    def new_asteroid(self, size, x, y):
        self.asteroids.append(Asteroid(screen, level, self, size, True, x, y))
...
class Spaceship(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if keys[pygame.K_UP]:
            self.velocity = min(self.velocity + self.acceleration, self.max_speed)  # Vorwärts beschleunigen
        elif keys[pygame.K_DOWN]:
            self.velocity = max(self.velocity - self.acceleration, -self.max_speed / 2)  # Rückwärts beschleunigen
        else:
            # Automatisches Abbremsen (wenn keine Taste gedrückt wird)
            if self.velocity > 0:
                self.velocity = max(self.velocity - self.friction, 0)  # Langsames Bremsen nach vorne
